  ...
  module Helpers
    module MootoolsHelper
     ...
     alias_method :form_remote_for, :remote_form_for
      ...
      class JavaScriptGenerator
        ...
        private
          ...
          module GeneratorMethods
            ...
            def inject(position, id,to, *options)
            	options = options.extract_options!
              insertion = position.to_s.camelize
              #call "$('#{id}').inject", to, position
              record "$('#{id}').inject(#{to}, '#{position}')"
             	#call "$('#{id}').insert", render(*options_for_render), insertion
              #record "#{arguments_for_call([render(*options_for_render)])}.toElement().inject($('#{id}'),'#{insertion}')"
            end
            ...
            def insert_html(position, id, *options_for_render)
              ...
              record "#{javascript_object_for(render(*options_for_render))}.stripScripts(true)"
             	#call "$('#{id}').insert", render(*options_for_render), insertion
              #record "#{arguments_for_call([render(*options_for_render)])}.toElement().inject($('#{id}'),'#{insertion}')"
            end
            ...
            #Remove an element
            def remove(id)
            	#loop_on_multiple_args "destroy()", ids
            	call "$('#{id}').destroy"
            end
            ...
            def replace(id, *options_for_render)
              ...
              call "$('#{id}').appendAfter", render(*options_for_render)
              call "$('#{id}').destroy"
              record "#{javascript_object_for(render(*options_for_render))}.stripScripts(true)"
            end            
      ...
      def dom_ready(script=nil,delay=nil)
        js =  "window.addEvent('domready', function() {"
        js << script
        js << "}#{!delay.nil? ? ".delay(#{delay})" : ''});"
        js
      end
